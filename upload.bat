...
REM Fetch and pull remote changes first
echo.
echo Fetching latest changes from GitHub...
git fetch origin main
if !ERRORLEVEL! NEQ 0 (
    echo WARNING: Failed to fetch from GitHub, continuing...
)

REM Check if we need to pull (if remote is ahead)
...
if !ERRORLEVEL! NEQ 0 (
    ...
    if !ERRORLEVEL! NEQ 0 (
        echo WARNING: Could not pull remote changes, but continuing to push...
    )
)
